    ...
    return Scaffold(
      ...
      body: newsNotifier.isLoading
          ...
          : ListView(
              children: [
                // Рисуем картинку
                newsPicture(newsNotifier.getNewsList()[routeArgs['newsCount']].imgURL),
                SizedBox(height: 10),
                ListView.separated(
                  ...
                  itemCount: newsNotifier.getNewsList()[routeArgs['newsCount']].desc.length,
                  itemBuilder: (BuildContext context, int newsBodyCount) {
                    return Text(newsNotifier.getNewsList()[routeArgs['newsCount']].desc[newsBodyCount].text);
                  },
                ),
              ],
            ),
